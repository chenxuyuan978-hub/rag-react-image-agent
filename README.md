# 基于 RAG + LangGraph 的图像处理论文复现实验分析 Agent

本项目面向图像处理论文复现实验分析场景，目标是辅助用户从论文片段中检索实验设置，运行本地图像处理实验，计算评价指标，生成实验报告，并通过前端和 API 查看实验结果。

项目当前定位是：基于 RAG + LangGraph 的图像处理论文复现实验分析 Agent，当前支持论文片段检索、实验配置抽取、图像处理实验运行、指标分析、报告生成，以及真实论文与源码接入任务管理。

项目结合了本地 RAG 检索、规则 ReAct 工具链、LangGraph 工作流、LLM 实验配置抽取、Streamlit 可视化、FastAPI 后端和 Docker 部署能力。当前定位是“辅助论文复现实验分析”，不是“自动复现任意论文”，也不是“任意论文代码一键跑通”。
...
- 真实论文与源码接入：支持论文 txt/md/pdf、本地源码目录和源码 zip，为每次复现任务创建独立 workspace。
...
真实论文/源码 -> Paper Loader / Source Ingestion -> Workspace + Summary
        |
        v
outputs / reports / metrics / run history / reproduction_runs
```
...
- Reproduction Intake 是第五阶段新增的真实论文与源码接入能力，暂时独立于原有 LangGraph 实验 workflow。
...
- `POST /api/reproduction/intake`：创建真实论文与源码接入任务。
- `GET /api/reproduction/runs`：获取复现接入任务列表。
- `GET /api/reproduction/runs/{run_id}`：获取某次复现接入任务 summary。
...
- 论文源码接入：输入真实论文路径和源码路径，创建独立 reproduction workspace，并展示 `intake_summary`。
- 项目说明：展示项目当前能力和用途。
...
第五阶段新增真实论文与源码接入能力，用于把系统从 demo 论文升级到真实复现任务材料管理。当前支持：

- 论文输入：`.txt`、`.md`、`.pdf`。
- 源码输入：本地源码目录、源码 `.zip`。
- 任务目录：`data/reproduction_runs/{run_id}/`。
- 子目录：`paper/`、`source/`、`artifacts/`、`logs/`。
- 输出文件：`paper_text.txt`、`intake_summary.json`。
- 前端页面：`论文源码接入`。
...
启动后端和前端后，可以在 Streamlit 的 `论文源码接入` 页面填写 `paper_path` 和 `source_path`，创建复现接入任务并查看 summary。
...
- 第四阶段：项目理解与复盘，包括项目展示材料、面试讲解材料和阶段复盘。
- 第五阶段：真实论文与源码接入，包括论文 txt/md/pdf 读取、源码 zip/目录接入、独立 reproduction workspace 和任务 summary。
- 第六阶段：源码分析与环境规划，后续计划包括依赖文件识别、README 运行命令提取和环境建议。
- 第七阶段：容器化部署与实验执行，后续计划包括隔离运行和任务日志采集。
- 第八阶段：错误诊断、自动修复与重试，后续计划包括更细粒度失败分析。
- 第九阶段：结果分析、指标对齐与复现报告，后续计划包括真实论文指标对齐。
- 第十阶段：企业级任务平台与可观测性，后续计划包括任务队列、权限、审计和更完整监控。

## 当前限制

- 当前主要支持 demo 图像处理实验和传统图像处理方法。
- 第五阶段已支持真实论文与源码材料接入，但不代表可以自动运行真实论文源码。
- PDF 解析依赖可选库 `pypdf` 或 `PyPDF2`，如果环境没有安装 PDF 解析库，系统会给出清晰错误提示。
- 当前 LLM 默认是 FakeLLM，不代表真实模型推理能力。
- 当前不能保证自动复现任意真实论文。
- 第五阶段不支持自动分析源码依赖、自动生成 Dockerfile、自动构建环境、自动执行真实论文源码、自动下载数据集或预训练权重、自动对齐论文指标。
- 真实论文代码复现仍需要处理数据集下载、依赖冲突、CUDA 环境、随机种子、评估协议、模型权重、代码安全和论文代码质量等问题。
- 深度学习图像复现模型尚未接入。
...
- [项目理解与复盘指南](docs/project_understanding_guide.md)
- [真实论文与源码接入说明](docs/reproduction_intake.md)
- [第五阶段总结](docs/phase_5_summary.md)
