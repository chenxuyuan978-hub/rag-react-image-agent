...
def analyze_metrics_node(state: GraphAgentState) -> dict[str, Any]:
    ...
    if state.get("error"):
        return {
            ...
            "steps": _append_step(
                ...
                node,
                "skipped",
                "Skipped because a previous node reported an error.",
            ),
        }
    ...
    return {
        "metrics_analysis": analysis,
        "steps": _append_step(state, node, "ok", "Metrics analyzed.", {"rows": rows}),
    }
...
def generate_report_node(state: GraphAgentState) -> dict[str, Any]:
    ...
    if state.get("error"):
        final_answer = f"LangGraph workflow failed: {state['error']}"
        return {
